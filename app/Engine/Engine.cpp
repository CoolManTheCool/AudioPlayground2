...
Engine_T::Engine_T() {
    ...
    window = glfwCreateWindow(mode->width * 3 / 4, mode->height * 3 / 4, "Audio Playground 2", nullptr, nullptr);
    if (!window) abort();
    glfwMakeContextCurrent(window);
    glfwSwapInterval(1); // Vsync

    // ---- Create ImGui context AFTER window is valid ----
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    frameTimes.resize(5 * mode->refreshRate);
}
